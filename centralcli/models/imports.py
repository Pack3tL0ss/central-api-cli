...
class _ImportSubDevice(BaseModel):
    ...
    subscription: str = Field(alias=AliasChoices("subscription", "license", "services", "SUBSCRIPTION"))
...
class ImportSubDevice(_ImportSubDevice):
    ...
    async def _get_confirm_msg(self, tags_override: bool = False, skipped: bool = False) -> str:
        # Believe we only need to skip a subscription update if the device is not assigned.  No update will work if the device doesn't exist
        # if not self.tags:
        #     return self.rich_help_text

        _tag_msg = ""
        if self.tags and not self.exists:  # tags can be updated as long as device exists no need for it to be assigned.
        ...
        if skipped and (not self.assigned or not self.exists):
            skip_for = "[bold]ALL[/] updates" if not self.exists else "subscription assignments"
            _error = "Not assigned to Aruba Central app in GLP." if self.exists else "Does not exist in GLP"
            return f"[dark_orange3]\u26a0[/]  [bright_red]SKIPPING[/] [turquoise2 italic]({skip_for})[/] {_ret}... [dim italic]{_error}[/]"
        else:
            return _ret
    ...
    def serials_by_subscription_id(self, assigned: bool = None) -> dict[str, BySubId]:
        ...
        for inv_dev, dev in zip(inv_devs, self.root):
            ...
            if assigned and inv_dev.assigned:
                ...
                out_dict[dev.sub.id].append(dev)
            elif assigned is None:  # All devices
                out_dict[dev.sub.id].append(dev)

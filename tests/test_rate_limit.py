...
from . import api_clients, log, test_data
...
def skip_test_rate_limit():
    ...
    b_resp = api.session.batch_request(b_reqs)
    end = time.perf_counter() - start
    _ = [print(log) for log in api.session.rl_log]
    _ = [print(f"{r.ts} tr:{r.remain_day} psr: {r.remain_sec} ({r.reason}): [{r.method}] {r.url}") for r in api.session.requests]
    print(f"Time Elapsed for all calls: {end:.2f}")
    print(b_resp[-1].rl.text)
    failed = [r for r in api.session.requests if not r.ok]
    successful_retries = [r for f in failed for r in api.session.requests if r.ok and r.url == f.url]

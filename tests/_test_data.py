...
from centralcli.typedefs import StrOrPath

TEST_FILE_DIR = config.cache_dir / "test_files"
...
def setup_cert_file(cert_path: str, sfx: str = "pem") -> Path:
    test_cert_file = TEST_FILE_DIR / f"test_runner_cert.{sfx}"
    file = Path(cert_path)
...
def setup_batch_import_file(test_data: dict | str, import_type: str = "sites", invalid: bool = False, duplicate: bool = False) -> Path:
    ...
    test_batch_file = TEST_FILE_DIR / f"test_runner_{import_type}{'' if not duplicate else '_w_dup'}{'' if not invalid else '_invalid'}.{sfx}"
...
def _create_banner_file(template: bool = True):
    if template:
        test_batch_file = TEST_FILE_DIR / "test_runner_ap_banner.j2"
        test_batch_file.write_text(banner_text_j2.lstrip())
    else:
        test_batch_file = TEST_FILE_DIR / "test_runner_ap_banner"
...
def _create_caas_commands_file(scope: Literal["groups", "sites", "gateways", "invalid", "empty"]) -> Path:
    commands_file = TEST_FILE_DIR / f"test_runner_caas_{scope}.yaml"
    if scope != "empty":
...
def _create_invalid_var_file(file: str, bad_json: bool = False) -> Path:
    var_file = Path(file)
    test_var_file = TEST_FILE_DIR / f"test_runner_invalid_variables{'_bad_json' if bad_json else ''}{var_file.suffix}"
...
def setup_deploy_file(group_file: Path, site_file: Path, label_file: Path, device_file: Path) -> Path:
    test_deploy_file = TEST_FILE_DIR / "test_runner_deploy.yaml"
...
def setup_j2_file() -> Path:
    test_j2_file = TEST_FILE_DIR / "test_runner_template.j2"
...
# dynamically generated test files either from test_data.yaml or with hard-coded content
test_outfile: Path = TEST_FILE_DIR / "test_runner_outfile"
test_invalid_empty_file: Path = TEST_FILE_DIR / "test_runner_empty_file"
...
test_cert_file: Path = setup_cert_file(cert_path=test_data["test_files"]["certificate"]["pem"])
test_cert_file_p12: Path = setup_cert_file(cert_path=test_data["test_files"]["certificate"]["p12"], sfx="p12")
test_cert_file_der: Path = setup_cert_file(cert_path=test_data["test_files"]["certificate"]["der"], sfx="der")
test_cert_file_invalid_sfx: Path = setup_cert_file(cert_path=test_data["test_files"]["certificate"]["pem"], sfx="abc")
test_var_file = _create_test_file_from_source_file(test_data["test_files"]["variable_file"])
test_invalid_var_file = _create_invalid_var_file(test_data["test_files"]["variable_file"])
test_invalid_var_file_bad_json = _create_invalid_var_file(test_data["test_files"]["variable_file"], bad_json=True)
...
gw_group_config_file = _create_test_file_from_source_file(test_data["test_files"]["gw_grp_config"])
test_ap_ui_group_template = _create_test_file_from_source_file(templates["j2"]["aos10_grp"]["template"])
test_ap_ui_group_variables = _create_test_file_from_source_file(templates["j2"]["aos10_grp"]["variables"])
test_sw_template = _create_test_file_from_source_file(templates["central"]["sw"]["template"])
